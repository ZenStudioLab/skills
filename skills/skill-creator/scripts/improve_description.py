...
by calling a provider CLI as a subprocess. Supports opencode, claude, codex, gemini.
"""
...
def _call_provider(
    prompt: str, model: str | None, provider: str = DEFAULT_PROVIDER, timeout: int = 300
) -> str:
    """Run a provider CLI with the prompt on stdin and return the text response.
    ...
    if provider == "opencode":
        ...
        cmd = ["codex", "-p", "--output-format", "text"]
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"{provider} -p exited {result.returncode}\nstderr: {result.stderr}"
        )
...
def improve_description(
    ...
    """Call provider to improve the description based on eval results."""
    failed_triggers = [
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _call_provider(prompt, model, provider)
    ...
    if len(description) > 1024:
        ...
        shorten_text = _call_provider(shorten_prompt, model, provider)
        match = re.search(
...
def main():
    ...
    parser.add_argument(
        ...
        choices=PROVIDERS,
        help=f"Provider CLI to use for improvement (default: {DEFAULT_PROVIDER})",
    )
    parser.add_argument(
